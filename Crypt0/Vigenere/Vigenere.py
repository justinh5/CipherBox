...
from Crypt0.Vigenere.VEnDecode import encode, decode
...
class Vigenere(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        ebutton.configure(command=lambda: self.encode_prep(tinput.get(1.0, 'end-1c'), key.get()))
        dbutton = Button(self, text="Decode", width=15)
        dbutton.grid(row=3, column=1, padx=10, sticky=W)
        dbutton.configure(command=lambda: self.decode_prep(tinput.get(1.0, 'end-1c'), key.get()))
    ...
    def encode_prep(self, plaintext, key):
    ...
    def decode_prep(self, ciphertext, key):
